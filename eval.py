...
import datetime
import pyfilecache
from inject_weight import inject_weight
...
def evaluate_no_hierarchy(data, num = 1000, with_dijkstra = False):
    ...
    # gens = [dijkstra_normalized(t) for t in tqdm(partial, desc = "Dijkstra for generation", unit = "trip", dynamic_ncols=True)]
    with multiprocessing.Pool() as pool, tqdm(total=len(partial), dynamic_ncols=True) as pbar:
        gens = [i.get() for i in [pool.apply_async(dijkstra_normalized, (t,), callback=lambda *_: pbar.update()) for t in partial]]
    ...
    generated = [(t,g) for t,g in generated if len(t)>3 and len(g)>3]
    lengths = [(trip_length(t[1:-1]), trip_length(g[1:-1])) for (t,g) in generated]
    inter_union = [intersections_and_unions(t[1:-1], g[1:-1]) for (t,g) in generated]
...
def load_model(path_model, path_extras):
    ...
    from torch import nn
    model = nn.Module.to(model, device)
    model.eval()
    print('Model loaded')
...
def dijkstra_normalized(true_trip):
    _, (src, *_, dest), (s, _) = true_trip
    g = transformed_graph
    path =  nx.dijkstra_path(g, src, dest, weight = "average_weight")
    path = [x for x in path]
    return path

def no_dijkstra_normalized_graph(args, transformed_graph, max_nbrs, model, dest):
    assert args.loss == "v2", "I dont think this will work for loss v1"
    # _, (src, *_, dest), (s, _) = true_trip
    g = transformed_graph
    with torch.no_grad():
    ...
    for u in g.nodes():
        for i,nbr in enumerate(node_nbrs[u]):
            if nbr == -1:
                break
            if "accumulated_weight" in g[u][nbr]:
                g[u][nbr]["accumulated_weight"] += transition_nll[count][i]
            else:
                g[u][nbr]["accumulated_weight"] = transition_nll[count][i]
            if "count" in g[u][nbr]:
                g[u][nbr]["count"] += 1
...
def worker(data):
    args, transformed_graph, max_nbrs, model, dests, fname = data
    for dest in tqdm(dests, desc = f"[worker {fname}]", dynamic_ncols=True):
        transformed_graph = no_dijkstra_normalized_graph(args, transformed_graph, max_nbrs, model, dest)
    with open(fname, 'wb') as f:
        pickle.dump(transformed_graph, f)
...
if True:
    ...
    CURRENT_TEMP = [c for c in transformed_graph.nodes()]
    CURRENT = [c for c in CURRENT_TEMP for _ in (node_nbrs[c] if c in node_nbrs else []) ]
    POT_NEXT = [nbr for c in CURRENT_TEMP for nbr in (node_nbrs[c] if c in node_nbrs else [])]

    if inject_weight(transformed_graph, args.dataset):
        ...
        if __name__ == "__main__":
            ...
            import os
            os.makedirs(f"pkls/{args.dataset}", exist_ok=True)
            torch_ctx = torch.multiprocessing.get_context("spawn")
            n_part = 6
            data = list(transformed_graph.nodes)
            with torch_ctx.Pool(n_part) as p:
                p.map(
                    worker,
                    [
                        (args, transformed_graph, max_nbrs, model, partial, f"pkls/{args.dataset}/{idx}.pkl")
                        for idx, partial in enumerate([data[i::n_part] for i in range(n_part)])
                    ]
                )